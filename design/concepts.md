...
*   **requires** The `user` exists and has an associated `Profile`. Callers must always provide the `newLearningGoals` argument; pass `undefined` when it should be cleared.
...
*   **effects** Adds the provided `users` to the `participants` set, ignoring duplicates or existing members.
