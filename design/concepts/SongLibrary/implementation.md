...
[@SongLibrary](SongLibrary.md)

# implement prompt

Implement the **SongLibrary** concept for the ChordConnect app.

Use the SongLibrary concept specification in `SongLibrary.md` as source of truth

Implement a TypeScript class `SongLibraryConcept` that:

- Manages a catalog of songs and their metadata.
- Lets users start/stop learning songs and track their mastery per song.
- Provides queries to:
  - List all songs.
  - List songs playable with a given set of chords.
  - Get chords and metadata for a given song.

# requirements

- Use the curated song dataset stored in `data/songs.json`.
...
* **state**:
  * `songs`: A collection of Song documents containing metadata and chord information.
  * `users`: A collection of User documents tracking the songs they are learning and their mastery level.
