...
1.  **Setup**: A user "Alice" and a Song Library with 5 songs are mocked.
2.  **Scenario 1 (Logic Check)**: Alice knows only "C". The engine evaluates "G" (unlocks Song A), "Am" (unlocks Song B), and "E" (unlocks Song E). "G" and "E" are tied for difficulty (1), but "E" comes first alphabetically. "Am" is harder (5). The engine recommends **E**.
3.  **Scenario 2 (Difficulty Tie-Break)**: Alice learns "E". Now knows "C", "E". Remaining candidates "G" and "Am". "G" unlocks Song A (diff 1). "Am" unlocks Song B (diff 5). "G" is easier. The engine recommends **G**.
4.  **Scenario 3 (Volume Priority)**: Alice learns "G". Now knows "C", "E", "G".
    *   Song A is fully playable (was C, G).
    *   Candidate "Am": Unlocks Song B (requires C, Am) AND Song D (requires C, G, Am). Total 2.
    *   Candidate "D": Unlocks Song C (requires G, D). Total 1.
    *   The engine prioritizes the count of unlocked songs. Recommendation is **Am**.
5.  **Scenario 4 (Completeness)**: Alice learns "D" and "Am". She knows everything required for all songs. The engine returns a specific "none" ID indicating no helpful recommendation exists.
6.  **Stateless Queries**: The stateless versions of the functions (`requestChordRecommendation`, `requestSongUnlockRecommendation`) are called to verify they return the same logical results as the stateful `calculateRecommendation` action, ensuring the logic is pure and consistent.