...
import { SongLibrary, ChordLibrary, Requesting, Sessioning, RecommendationEngine } from "@concepts";
 ...
 */
export const HandleChordAdditionRequest: Sync = ({ request, sessionId, chord, user }) => ({
  when: actions(
    [Requesting.request, { path: "chords/add", sessionId, chord }, { request }]
  ),
  where: async (frames) => {
    return await frames.query(Sessioning._getUser, { session: sessionId }, { user });
  },
  then: actions(
    [ChordLibrary.addChordToInventory, { user, chord, mastery: "in-progress" }]
  )
 ...
 */
export const RespondToChordAddition: Sync = ({ request, user, chord, response }) => ({
  when: actions(
  ...
  where: async (frames) => {
    ...
    for (const frame of frames) {
        ...
        const playableSongsObjs = await SongLibrary._getPlayableSongs({ knownChords: knownChordsList });
        ...
        let recommendation: { recommendedChord: string | null; unlockedSongIds: ID[] } = { recommendedChord: null, unlockedSongIds: [] };
        if (recs.length > 0) {
             const recChord = recs[0].recommendedChord;
             const unlocks = await RecommendationEngine.requestSongUnlockRecommendation({
                 knownChords: knownChordsList,
 ...
 */
export const GetChordInventory: Sync = ({ request, sessionId, user, inventory }) => ({
  when: actions(
